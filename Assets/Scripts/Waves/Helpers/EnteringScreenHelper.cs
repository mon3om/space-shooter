...
public class EnteringScreenHelper
{
    ...
    public static void EnterFromRandom(List<Transform> transforms)
    {
        foreach (var item in transforms)
        {
            Vector2 position = new(Random.Range(CameraUtils.CameraRect.xMax, 2 * CameraUtils.CameraRect.xMax),
            Random.Range(CameraUtils.CameraRect.yMax, CameraUtils.CameraRect.yMax * 2));
            position.x *= Random.value > 0.5f ? 1 : -1;
            position.y *= Random.value > 0.5f ? 1 : -1;

            float xMax = CameraUtils.CameraRect.xMax;
            float yMax = CameraUtils.CameraRect.yMax;
            Vector2 targetPosition = new(Random.Range(-xMax + 2, xMax - 2), Random.Range(-yMax + 1, yMax - 1));
