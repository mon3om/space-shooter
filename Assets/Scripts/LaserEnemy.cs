...
public class LaserEnemy : EnemyAIBase
{
    ...
    void Start()
    {
        ...
        if (state != 0) // State 1 and 2
        {
            ...
            directionalMover.movementSpeed *= 2;
    ...
    private void Update()
    {
        if (state == 0 && laserInstance)
        {
            ...
            laserInstance.SetEndpoint(new(laserInstance.EndPoint.x, transform.position.y), false);
        }
        if (laserInstance)
        {
            ModifyLaserLength();
    ...
    private void OnDestinationReachedState1()
    {
        ...
        laserInstance.SetEndpoint(transform.position - Vector3.up * (CameraUtils.CameraRect.yMax + transform.position.y), false);
    ...
    private void OnDestinationReachedState2()
    {
        ...
        laserInstance.emissionSpeed /= 2;
        laserInstance.StartPoint = transform.position;
        laserInstance.SetEndpoint(targetPoint, false);
        directionalMover.movementSpeed /= 2;
    }
    ...
    private void OnDestinationReachedState0()
    {
        InstantiateLaser();
        laserInstance.StartPoint = transform.position;
        laserInstance.SetEndpoint(transform.position + Vector3.left * Mathf.Sign(transform.position.x) * (CameraUtils.CameraRect.xMax + Mathf.Abs(transform.position.x)), false);
    ...
    private void ModifyLaserLength()
    {
        if (isModifying)
        {
            if (isIncreasing)
            {
                ...
                laserInstance.SetEndpoint(end, false);
            }
            else
            {
